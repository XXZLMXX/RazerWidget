package com.lucifer.razerlib.imageview.screenshot;
...
public class ScreenshotTaker
{
	 ...
	 * 
	 */
	public ScreenshotTaker(ActivityUtils activityUtils, ViewFetcher viewFetcher,
			Sleeper sleeper)
	{
	...
	private View getScreenshotView()
	{
		...
		while (decorView == null)
		{
			...
			sleeper.sleepMini();
			decorView = viewFetcher.getRecentDecorView(viewFetcher
					.getWindowDecorViews());
		}
	...
	{
		ArrayList<GLSurfaceView> currentViews = viewFetcher.getCurrentViews(
				GLSurfaceView.class, decorView);
		...
		for (GLSurfaceView glView : currentViews)
		{
			...
			if (renderer == null)
			{
				renderer = new Reflect(glView).field("mRenderer").out(
						Renderer.class);
	...
	private class ScreenShotSaver extends Handler
	{
		...
		private void saveFile(String name, Bitmap b, int quality)
		{
			...
			File directory = new File("/sdcard/screenshot");
			directory.mkdir();
