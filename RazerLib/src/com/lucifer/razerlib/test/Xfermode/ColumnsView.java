...
public class ColumnsView extends View
{
	...
	private void drawColumns(Canvas canvas)
	{
		...
		{
			mColumnPath.reset();
			mColumnPath.moveTo(i * mStartX + mStartX * 3 / 2 + mSpace,
					19 * mStartY);
			mColumnPath.lineTo(i * mStartX + mStartX * 5 / 2 - mSpace,
					19 * mStartY);
			mColumnPath.lineTo(i * mStartX + mStartX * 5 / 2 - mSpace, 19
					* mStartY - mColumnHeiList.get(i));
			mColumnPath.lineTo(i * mStartX + mStartX * 3 / 2 + mSpace, 19
					* mStartY - mColumnHeiList.get(i));
			mColumnPath.close();
			// 设置画笔随机颜色
			...
			canvas.drawText("NO." + i, i * mStartX + mStartX * 3 / 2 + mSpace,
					19 * mStartY - mColumnHeiList.get(i), mPaint);
			// mColumnPaint.reset();
		}
		canvas.restore();
	...
	private void drawUnitLength(Canvas canvas)
	{
		...
		for (int i = 1; i < 20; i++)
		{// 绘制Y轴单位标记
			canvas.drawLine(mStartX - 15, mStartY * i, mStartX, mStartY * i,
					mPaint);
		}
		for (int i = 1; i < 20; i++)
		{// 绘制X轴单位标记
			canvas.drawLine(mStartX * i, 19 * mStartY + 15, mStartX * i,
					19 * mStartY, mPaint);
		}
		for (int i = 19; i > 1; i--)
		{// 绘制Y轴单位数字
			canvas.drawText(mStartY * (19 - i) + "", mStartX - 80, mStartY * i
					+ 15, mPaint);
		}
		for (int i = 19; i > 1; i--)
		{// 绘制X轴单位数字
			canvas.drawText(mStartX * (19 - i) + "", (20 - i) * mStartX,
					19 * mStartY + 50, mPaint);
		}
