...
public class MatrixImageView extends ImageView
{
	...
	private void init()
	{
		...
		bitmap = Bitmap.createScaledBitmap(bitmap,
				DimenUtil.getScreenSize((Activity) mContext)[0],
				DimenUtil.getScreenSize((Activity) mContext)[1], true);
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_MOVE:// 前面的操作都是计算和存储，这里触摸点移动时重绘
			/*
			 * 单点触控拖拽平移
			 */
			if (mode == MODE_DRAG)
			...
			else if (mode == MODE_ZOOM && event.getPointerCount() == 2)
			{
				...
				if (preEventCoor != null)
				{
					...
					currentMatrix.postRotate(r, getMeasuredWidth() / 2,
							getMeasuredHeight() / 2);
				}
			}
			break;
